...
//	}
	...
	@Primary
	@Bean(name = "tqrDataSource")
	@ConfigurationProperties(prefix = "spring.datasource.tqr")
	public DataSource getDateSource1() {
		return DataSourceBuilder.create().build();
	}
	...
	@Bean(name = "dynamicDataSource")
	public DynamicDataSource DataSource(@Qualifier("tqrDataSource") DataSource tqrDataSource,
										@Qualifier("spiderDataSource") DataSource spiderDataSource) {
		Map<Object, Object> targetDataSource = new HashMap<>();
		targetDataSource.put(DataSourceType.DataBaseType.TEST01, tqrDataSource);
		targetDataSource.put(DataSourceType.DataBaseType.TEST02, spiderDataSource);
		DynamicDataSource dataSource = new DynamicDataSource();
		dataSource.setTargetDataSources(targetDataSource);
		dataSource.setDefaultTargetDataSource(tqrDataSource);
		return dataSource;
	}
